...
class IndexHandler:
    ...
    def create_partial_value_indexes(self, **kwargs):
        ...
        max_memory_allowed = kwargs.get('max_memory', 12)
    ...
    def merge_partial_value_indexes_and_process_max_frequency(self):
        ...
        filenames = {f'{self.config.dataset_config["dataset_directory"]}{m.groups()[0]}' for f in os.listdir(self.config.dataset_config['dataset_directory']) if (m:= search(r'(value_index_file.shelve.part\d+).*', f))}
        ...
        with ExitStack() as stack:
            ...
            final_value_index = stack.enter_context(shelve.open(f'{value_index_path}', flag='n'))
