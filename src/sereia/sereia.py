...
class Sereia():
    ...
    def keyword_search(self, keyword_query=None, **kwargs):
        ...
        for _ in range(repeat):
            ...
            # topk = 10
            # print(f'Top-{topk} Query Matches')
            # pp(ranked_query_matches[:10])
            ...
            if not skip_cn_generations:
                ...
                # print(f'Top-{topk} CJNs')
                # pp(ranked_cns[:topk])
                # print(50 * '=')

                if len(ranked_cns) > 0:
                    top_cn = ranked_cns[0]
                    print('top cn: {}'.format(top_cn))
                    if self.assume_golden_standards_in_topk:
                        ...
                        # print('Golden standard position: {}'.format(golden_standard_position))
