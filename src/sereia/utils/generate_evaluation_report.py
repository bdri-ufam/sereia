...
INPUT_DATA_FILES = [
    ('../../../results/imdb.json', 'imdb.json',),
    # ('../../../results/nba.json', 'nba.json',),
    # ('../../../results/dblp.json', 'dblp.json',),
    # ('../../../results/twitter.json', 'twitter.json',),
    ('../../../results/yelp_expanded.json', 'yelp_expanded.json',),
]
...
for file, dataset_name in INPUT_DATA_FILES:
    ...
    with open(f'parsed_results_{dataset_name}.csv', 'w') as f:
        ...
        for n, result in enumerate(results):
            ...
            # print('QM Count: ', data['qm_count'])
            print('CN Count: ', data['cn_count'])

            # min_cn_count = SYS_MAXSIZE
            max_cn_count = max(max_cn_count, data['cn_count'])
            avg_cn_count_l.append(data['cn_count'])

            max_qm_count = max(max_qm_count, data['qm_count'])
            avg_qm_count_l.append(data['qm_count'])
        ...
        print(f'Max CN Count: {max_cn_count} | Avg CN Count: {sum(avg_cn_count_l)/len(avg_cn_count_l)}')
        print(f'Max QM Count: {max_qm_count} | Avg QM Count: {sum(avg_qm_count_l)/len(avg_qm_count_l)}')
